...
def soft_lcscl(hidden,
    ...
    masks = F.one_hot(torch.arange(batch_size), num_classes=batch_size).to(hidden.device)
    
    # Pairwise Similarity 
    sim_aa = - distance(hidden1, hidden1_large, dist, pairwise=True) / temperature
    sim_bb = - distance(hidden2, hidden2_large, dist, pairwise=True) / temperature
    sim_ab = - distance(hidden1, hidden2_large, dist, pairwise=True) / temperature
    sim_ba = - distance(hidden2, hidden1_large, dist, pairwise=True) / temperature
    sim_aa = sim_aa - masks * LARGE_NUM
    sim_bb = sim_bb - masks * LARGE_NUM
    ...
    """ Attention here, still don't know why doing in this way cannot work 
        logits_1 = tf.concat([sim_aa, sim_ab], axis=1)
        logits_2 = tf.concat([sim_ba, sim_bb], axis=1)
        logits = tf.concat([logits_1, logits_2], axis=0)
        probs = tf.nn.softmax(logits, axis=1)
    """
    _logits_1 = torch.cat([sim_aa, sim_ab], dim=1)
    _logits_2 = torch.cat([sim_ba, sim_bb], dim=1)
    logits_12 = torch.cat([_logits_1, _logits_2], dim=0)
    ...
    # max_logits, _ = torch.max(logits_masked, dim=-1)
    # logits_masked = logits_masked - max_logits

    logits_1, logits_2 = torch.split(logits_masked, batch_size, dim=0)
    logprobs_1 = F.log_softmax(logits_1, dim=1)
    logprobs_2 = F.log_softmax(logits_2, dim=1)
    logprobs = torch.cat([logprobs_1, logprobs_2], dim=0) # [2b, 2b]
